...
exports.makeJudge0Submission = async (body) => {
    ...
    var result = null;
    await axios.post('https://emkc.org/api/v2/piston/execute', body)
        .then((res) => {
            result = res.data;
        })
        .catch((err) => {
...
exports.checkInputFormatHelper = async (qID, inputFile, language) => {
    const problemAllData = await problemsModel.findOne({ qID: qID });
    const judge0ApiInput = {
        language: language,
        version: "10.2.0",
        files: [{
          content: problemAllData.checkerProgram
        }],
        stdin: inputFile
